...
cp "$REPO_DIR/SemanticProvider.py"         "$BIN/"
cp "$REPO_DIR/AIAgentAction.py"            "$BIN/"
...
echo ""
echo "For SemanticRules (free local vector classification), the first classified file"
echo "downloads the embedding model (~240MB) to ~/.cache/folder-actions/fastembed."
